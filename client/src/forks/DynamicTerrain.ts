...
function gaussBlur_4(scl: number[], tcl: number[], w: number, h: number, r: number) {
...
const blur = (array: number[], resolution: number) => {
    return array
    const source: [number[], number[], number[], number[]] = [[], [], [], []];
    const target: [number[], number[], number[], number[]] = [[], [], [], []];
    array.forEach((x, i) => {
        const num = i % 4;
        source[num].push(x * 1000)
    })

    gaussBlur_4(source[0], target[0], resolution, resolution, 2);
    gaussBlur_4(source[1], target[1], resolution, resolution, 2);
    gaussBlur_4(source[2], target[2], resolution, resolution, 2);
    gaussBlur_4(source[3], target[3], resolution, resolution, 2);

    const newArray = [];
    for (let i = 0; i < array.length; i++) {
        const which = i % 4;
        const n = Math.floor(i / 4)
        newArray.push(target[which][n] / 1000)
    }
    return newArray
}
