...
import { IPhysicsEnginePlugin, PhysicsImpostorJoint } from "@babylonjs/core/Physics/IPhysicsEngine";
import { PhysicsEngine } from "@babylonjs/core/Physics/physicsEngine";
import { IPhysicsEnabledObject, PhysicsImpostor } from "@babylonjs/core/Physics/physicsImpostor";
import { DistanceJointData, IMotorEnabledJoint, PhysicsJoint, SpringJointData } from "@babylonjs/core/Physics/physicsJoint";
...
export class CannonJSPlugin implements IPhysicsEnginePlugin {
    ...
    public constructor(private _useDeltaForWorldStep: boolean = true, iterations = 10, cannonInjection = CANNON) {
        ...
        if (meshChildren.length) {
            const processMesh = (mesh: AbstractMesh) => {
                ...
                mesh.getChildMeshes(true)
                    .filter((m: any) => !!m.physicsImpostor)
                    .forEach(processMesh);
            };
            meshChildren.filter((m: any) => !!m.physicsImpostor).forEach(processMesh);
        }
    }
