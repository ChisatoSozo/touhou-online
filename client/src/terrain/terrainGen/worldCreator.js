...
export function worldCreator(cx, cy, perlinSeed, rands) {
    ...
    var sum = 0;
    for (let kk = 0; kk < rands[0]; kk++) {
        sum += Math.exp(
            -(Math.pow(ccx - 0.8 * rands[kk + 1], 2) + Math.pow(ccy - 0.7 * rands[kk + 7], 2)) / (2 * 0.03 * rands[kk + 13]),
        );
    }
    ...
    for (let kk = 2; kk <= 6; kk += 2) {
        const q = Math.pow(2, kk);
        sum += (2 * (noise.perlin2(ccx * q, ccy * q) + 1)) / 2 / q;
    }
    // Perlin plus some extra elevation for making an island (arbitrary)
    const boundary =
        Math.abs(1.05 * ccx) >= 1 || Math.abs(1.1 * ccy * 2) >= 1
            ? 0
            : Math.pow(Math.abs(Math.cos((1.05 * ccx * Math.PI) / 2) * Math.cos(1.1 * ccy * Math.PI)), 0.5);
    iter = sum;
    const basic = mount * continental + 2 * continental + mount * 0.75 + Math.max(continental, mount);
    iter = basic + 12 * iter + 2 * basic * iter;
    iter = iter * (1 + boundary);
    return iter;
}

export function elevationRescaling(iter, sea, max) {
...
export var weightedFunction = function (cx, cy, maxIter, grid, correlationDistance) {
    var zAverage = 0;
    var module = 0;
...
export var linearFunction = function (cx, cy, maxIter, grid, dx) {
    let iter;
    if (grid[0] && grid[1] && grid[2] && grid[3]) {
        ...
        let x = cx - grid[0].x;
        let y = cy - grid[0].y;
        let x10 = grid[1].x - grid[0].x;
        let y10 = grid[1].y - grid[0].y;
        let x11 = grid[2].x - grid[0].x;
        let y11 = grid[2].y - grid[0].y;
        let x01 = grid[3].x - grid[0].x;
        let y01 = grid[3].y - grid[0].y;
        let points = [
            [x, y],
            [x10, y10],
            [x11, y11],
        ...
        iter = z00 + r * (-z00 + z10) + s * (-z00 + z01) + r * s * (z00 - z10 - z01 + z11);
...
function reline4(points) {
    ...
    for (let i = 0; i < points.length; i++) {
        ...
        if (points[i][1] / points[i][0] > y110 / x110) {
            px = points[i][0] / x110;
            py = (x110 * points[i][1] + points[i][0] * (1 - y110)) / x110;
        } else {
            py = points[i][1] / y110;
            px = (y110 * points[i][0] + points[i][1] * (1 - x110)) / y110;
        }
        points0.push([px, py]);
    }
    return points0;
...
export var linearPixelFunction = function (cx, cy, maxIter, grid, dx) {
    let iter;
    if (grid[0] && grid[1] && grid[2] && grid[3]) {
        // linear interpolation
...
// Erosion function
export function rivers(grid) {
    var le = grid.length;
    var gridS = Math.sqrt(le / 2);
    var ffsea = 0;
    for (var tyu = 2; tyu > 0; tyu--) {
        for (let k = le - 1; k >= 0; k--) {
            if (grid[k].type == 0) {
                var kn = k;
                var count = 1;
                var path = [];
                path.push(kn);
                let kk = Math.floor(Math.random() * 3) + 1;
                grid[kn].flux += kk;
                grid[kn].type = 1;
                let end = 0;
                while (end == 0) {
